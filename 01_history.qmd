...
references <- tribble( ~start_date, ~event, ~displ, ~alpha, ~image,
                ymd(19830101), "", -0.1, 0.2,NA_character_,
                ymd(19830101), "Latex", -0.1, 1,NA_character_,
                ymd(19840101), "Knuth DE, Literate Programming", 0.9, 1,NA_character_,
                ymd(19940101), "Leslie Lamport, LaTeX: A Document Preparation System", 0.8, 0.,"https://upload.wikimedia.org/wikipedia/commons/thumb/9/92/LaTeX_logo.svg/langfr-220px-LaTeX_logo.svg.png",
...
timeline <- bind_rows(references, packages) %>% ggtimeline %>% shift_axis(ymd(20130101), ymd(20241101))
timeline
```
...
timeline <- bind_rows(references, packages %>% filter(lang == "R"), outputs) %>%
  ggtimeline %>% shift_axis(ymd(19830101), ymd(20241101))
timeline
```
